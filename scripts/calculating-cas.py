...
def main():
    ...
    df_paths = (
        df
        .assign(hec_paths = df.id_str.apply(lambda x: f'/mmfs1/scratch/hpc/60/desi-{gal_type}/{x}-cutout.fits'))
    )

    a_dict = {i.replace('-cutouts.fits', '') : calc_a(i) for i in tqdm(list(df_paths.hec_path))}

    df_a = pd.DataFrame(a_dict, orient = 'index').reset_index().rename(columns = {'index' : 'id_str'})
